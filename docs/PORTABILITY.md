...
| 平台 | 证据 | 结论 |
| --- | --- | --- |
| macOS | 本地 arm64 真实 profiles、隔离升级、失败回滚、Web 进程恢复与 API smoke；公开 CI 重跑全部测试、双语 smoke、pack 和真实 DSH profile E2E | 已支持 |
| Linux | 本地 Debian/Node 24 arm64 容器完整检查与真实 DSH E2E；公开 Ubuntu runner 重跑全部测试、双语 smoke、pack 和真实 DSH profile E2E | 已支持 |
| Windows x64 | 原生 runner 验证 `.cmd` 实参、CIM 发现、端口 PID 定位、detached 启停、launcher 回收、双语 smoke、pack 和真实 DSH profile E2E | 已支持 |

Linux 当前依赖 procps 兼容的 `ps`。`lsof` 只用于尽力恢复进程工作目录，缺失时会回退到当前目录。公开 E2E 覆盖真实 DSH 安装、Web profile 初始化、状态检查和配置合成；隔离安装、原子切换、失败回滚及服务恢复由事务测试覆盖。CI 尚未对三种系统分别长期运行一个真实 Web 服务并故意制造切换失败，因此这仍是支持边界，不应被描述为穷尽了所有插件运行时组合。
...
4. 原生 CI 已证明 `.cmd` 实参不被解释、目录切换/回滚可用、detached 服务能被发现并停止，且 launcher 不残留。macOS 上的 platform mock 没有被当作这项验收证据。
